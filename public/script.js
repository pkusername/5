document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchBookings() {
        fetch('https://beihaihuanqiulvxingshe.xyz/api/bookings')
            .then(response => response.json())
            .then(bookings => {
                console.log('获取到的预订数据:', bookings); // 这里打印了你看到的数据
                ...
                bookings.forEach(booking => {
                    ...
                    row.innerHTML = `
                        ...
                        <td>${formatNumber(booking.totalPrice)}</td>
                        <td>${formatNumber(booking.otherFees)}</td>
                        <td>${formatNumber(booking.deposit)}</td>
                        <td>${formatNumber(booking.receivable)}</td>
    ...
    window.openEditModal = function(bookingId) {
        ...
        for (let row of rows) {
            ...
            if (editButton && editButton.getAttribute('onclick').includes(bookingId)) {
                ...
                booking = {
                    ...
                    rooms: parseRoomTypes(cells[4].textContent),
                    price: cells[8].textContent,
                    totalPrice: cells[9].textContent,
                    otherFees: cells[10].textContent,
                    deposit: cells[11].textContent,
                    receivable: cells[12].textContent,
                    guestCount: cells[13].textContent,
                    pickupTime: cells[14].textContent,
                    dropoffTime: cells[15].textContent
                };
                break;
            }
        }
        ...
        document.getElementById('otherFees').value = booking.otherFees || '';
        document.getElementById('deposit').value = booking.deposit || '';
        document.getElementById('receivable').value = booking.receivable || '';
    ...
    document.getElementById('bookingForm').addEventListener('submit', async function(e) {
        ...
        // 收集所有表单数据
            ...
            totalPrice: document.getElementById('totalPrice').value ? parseFloat(document.getElementById('totalPrice').value) : null,
            otherFees: document.getElementById('otherFees').value ? parseFloat(document.getElementById('otherFees').value) : null,
            deposit: document.getElementById('deposit').value ? parseFloat(document.getElementById('deposit').value) : null,
            receivable: document.getElementById('receivable').value ? parseFloat(document.getElementById('receivable').value) : null,
            guestCount: document.getElementById('guestCount').value ? parseInt(document.getElementById('guestCount').value) : null,
            pickupTime: document.getElementById('pickupTime').value.trim() || null,
            dropoffTime: document.getElementById('dropoffTime').value.trim() || null,
        ...
        fetch(url, {
            method: method,
                ...
                'Origin': 'https://beihaihuanqiulvxingshe.xyz'  // 添加 Origin 头
                },
                body: JSON.stringify(formData)
            })
            .then(response => response.json())
            .then(data => {
            alert(isEdit ? '修改成功！' : '预订成功！');
            ...
            .catch(error => {
            console.error(isEdit ? '修改失败:' : '预订失败:', error);
            alert(isEdit ? '修改失败，请重试' : '预订失败，请重试');
        });
    });
